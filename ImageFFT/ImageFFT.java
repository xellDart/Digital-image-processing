...
public class ImageFFT {
    ...
    public static double bartlettWindow(double r, double rmax) {
        return 1.0 - Math.min(r, rmax) / rmax;
    }
    
    public static double hammingWindow(double r, double rmax) {
        double f = (rmax - Math.min(r, rmax)) / rmax;
        return 0.54 - 0.46 * Math.cos(f * Math.PI);
    }

    public static double hanningWindow(double r, double rmax) {
        double f = (rmax - Math.min(r, rmax)) / rmax;
        return 0.5 - 0.5 * Math.cos(f * Math.PI);
    }

    public static double butterworthLowPassFunction(
            int n, double radius, double r) {
        double p = Math.pow(r / radius, 2.0 * n);
        return 1.0 / (1.0 + p);
    }
    
    public static double butterworthHighPassFunction(
            int n, double radius, double r) {
        try {
    ...
    public static double butterworthBandPassFunction(
            int n, double radius, double delta, double r) {
        return 1.0 - butterworthBandStopFunction(n, radius, delta, r);
    }

    public static double butterworthBandStopFunction(
            int n, double radius, double delta, double r) {
        try {
    ...
    private static int shift(int d, int d2) {
